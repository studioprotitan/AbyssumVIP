 ...
 * Compliance: SSOT Field Parity (assetId / babylonPath).
 */
...
export class FreightLoader {
  ...
  public static async loadChunk(scene: BABYLON.Scene): Promise<void> {
    ...
    for (const asset of confirmed) {
      if (!asset.babylonPath) {
        console.warn(`[MOAI:FREIGHT] Asset ${asset.assetId} missing babylonPath.`);
        continue;
      }

      try {
        console.log(`[MOAI:LOAD] Spawning asset: ${asset.assetId}`);
        
        // Path splitting for Babylon SceneLoader compliance
        const path = asset.babylonPath;
        ...
        if (result && result.meshes) {
          ...
          console.log(`[MOAI:SUCCESS] Asset active: ${asset.assetId}`);
        }
      } catch (err) {
        console.error(`[MOAI:ERROR] Failed to spawn ${asset.assetId}:`, err);
      }
    }
  }
}