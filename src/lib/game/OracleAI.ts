 ...
 * 
 * Enforces: "Signal degrades with distance... avoid Rift Rat detection."
 */
...
export class OracleAI {
  ...
  // Narrative constraints (distance in meters)
  ...
  public evaluateSignalProximity(distance: number) {
    ...
    if (distance > this.SIGNAL_THRESHOLD) {
      ...
      integrity = Math.max(0.05, 1 - ((distance - this.SIGNAL_THRESHOLD) / (this.SIGNAL_CRITICAL - this.SIGNAL_THRESHOLD)));
  ...
  public getTurretComm(risk: number): string {
    if (risk > 0.9) return "SIGNAL PATTERN STRETCHED. RAT SCOUTS DETECTED. REFORGING IMMINENT.";
    if (risk > 0.7) return "PULSE WEAKENING. YOU'D RATHER BE DEAD THAN REFORGED AS A SCOUT.";
    if (risk > 0.4) return "REMAIN WITHIN PROXIMITY. AVOID SIGNAL BLEED.";
